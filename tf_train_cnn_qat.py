...
def get_images(file_paths):
    ...
    for file_path in file_paths:
        with open(file_path, 'rb') as image_file:
            data = image_file.read()  # 파일 전체를 읽습니다.
            arr = np.frombuffer(data, dtype=np.uint8)
            # 파일의 전체 픽셀 수로부터 한 변의 길이를 추론 (정사각형 가정)
            dim = int(np.sqrt(arr.size))
            if dim * dim != arr.size:
                raise ValueError(f"File {file_path} size {arr.size} is not a perfect square. Cannot infer image dimensions.")
            image = arr.reshape((dim, dim))
            image = image / 255.0
            images.append(image)
...
def load_yuv_images(folder_path, file_names, image_size=None):
    """
    image_size가 None이면 파일의 전체 크기로부터 정사각형 해상도를 추론하고,
    image_size가 지정되었다면 해당 해상도로 reshape 합니다.
    """
    images = []
    for file_name in file_names:
        file_path = os.path.join(folder_path, file_name)
        with open(file_path, 'rb') as file:
            ...
            if image_size is None:
                # 데이터 길이가 정사각형 형태인지 확인하고 차원을 추론
                dim = int(np.sqrt(data.size))
                if dim * dim != data.size:
                    raise ValueError(f"File size {data.size} is not a perfect square. Cannot infer image dimensions.")
                image = data.reshape((dim, dim))
            else:
                # 사용자가 지정한 image_size로 reshape (예: (256, 256) 또는 (128, 128))
...
def main(args):
    ...
    for fold_index, (train_index, val_index) in enumerate(kf.split(date_folders)):
        ...
        if args.save_best:
            checkpoint_path = os.path.join(args.models_dir, f"Fold_{fold_index + 1}_qat_model.h5")
            model_checkpoint = ModelCheckpoint(checkpoint_path, monitor='val_accuracy', save_best_only=True, verbose=1)
            callbacks.append(model_checkpoint)

        # QAT 모델 훈련
        qat_model.fit(X_train, y_train, epochs=args.epochs, verbose=1, batch_size=64, validation_data=(X_val, y_val), callbacks=callbacks)
        
        # 모델 저장 (save_best 옵션이 활성화되지 않은 경우 마지막 모델 저장)
        if not args.save_best:
            model_save_path = os.path.join(args.models_dir, f"Fold_{fold_index + 1}_qat_model.h5")
            qat_model.save(model_save_path)
        else:
        ...
        saved_model_path = os.path.join(args.models_dir, f"Fold_{fold_index + 1}_qat_saved_model")
        qat_model.save(saved_model_path, save_format='tf')
        print(f"Fold {fold_index + 1}: Model also saved in SavedModel format at {saved_model_path}")

        # Evaluate the QAT model
        val_loss, val_acc = qat_model.evaluate(X_val, y_val)
        ...
        y_pred = qat_model.predict(X_val)
        y_pred = np.argmax(y_pred, axis=1)
        y_test = np.argmax(y_val, axis=1)
        result = confusion_matrix(y_test, y_pred)
        print(result)

        # TFLite 모델 변환 - quantization 모드를 args에서 선택
        if args.quantization_mode == 'int8':
        ...
        elif args.quantization_mode == 'fp16':
            converter = tf.lite.TFLiteConverter.from_keras_model(qat_model)
            converter.optimizations = [tf.lite.Optimize.DEFAULT]
            converter.target_spec.supported_types = [tf.float16]
            tflite_model = converter.convert()
        else:
            raise ValueError("Unsupported quantization mode selected.")

        # Save the tflite model
        tflite_save_path = os.path.join(args.models_dir, f"Fold_{fold_index + 1}_qat_model_{args.quantization_mode}.tflite")
...
def parse_arguments():
    ...
    parser.add_argument('--models_dir', type=str, default='/home/jijang/ssd_data/projects/Bacteria/models/case_test/2025_case16_qat_fp16', help='Directory where models are saved.')
    ...
    # 새로 추가된 인자: quantization_mode (int8 또는 fp16 선택)
    parser.add_argument('--quantization_mode', type=str, choices=['int8', 'fp16'], default='fp16', help='Quantization mode for TFLite conversion (int8 or fp16)')

...
class DistillationLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        distillation_loss = tf.keras.losses.KLDivergence()(soft_teacher_pred, soft_student_pred)
        hard_loss = tf.keras.losses.categorical_crossentropy(y_true, y_pred)
        return self.alpha * distillation_loss + (1 - self.alpha) * hard_loss

def load_distilled_model(model_path):
    with tfmot.quantization.keras.quantize_scope({
        'DistillationLoss': DistillationLoss  # 사용자 정의 loss 포함
    }):
...
def model_inference(X_test, tflite_model_path):
    ...
    모델의 입력 dtype가 int8인 경우 배치 전체에 대해 양자화하고,
    출력도 동일하게 후처리합니다.
    """
    # TFLite 인터프리터 생성
    interpreter = tf.lite.Interpreter(model_path=tflite_model_path)
    ...
    # 입력 데이터 전처리: 모델의 입력 dtype에 따라 양자화 처리
    if input_details[0]['dtype'] == np.int8:
        input_scale, input_zero_point = input_details[0]['quantization']
        # 전체 배치에 대해 벡터화 연산으로 양자화 진행
    ...
    # 출력 dtype가 int8이면, dequantize 처리
    if output_details[0]['dtype'] == np.int8:
        output_scale, output_zero_point = output_details[0]['quantization']
        output_data = (output_data.astype(np.float32) - output_zero_point) * output_scale

    # 각 샘플별 예측 결과를 np.argmax를 통해 반환 (배치 크기만큼의 1D array)
...
def main(args):
    ...
    for fold_index, (train_index, test_index) in enumerate(kf.split(date_folders)):
        ...
        model_path = os.path.join(args.student_model_dir, f"Fold_{fold_index + 1}_qat_model.h5")
        ...
        # TFLite 모델 파일명은 지정한 quantization 타입에 따라 결정
        tflite_model_filename = f"Fold_{fold_index + 1}_qat_model_{args.quantization}.tflite"
        ...
        if os.path.exists(tflite_model_path):
            ...
            print(f"Fold {fold_index + 1}: TFLite Quantization Type: {args.quantization.upper()}")
        else:
            print(f"Fold {fold_index + 1}: TFLite model not found at {tflite_model_path}")
...
def parse_arguments():
    ...
    parser.add_argument('--student_model_dir', type=str, default='/home/jijang/ssd_data/projects/Bacteria/models/case_test/2025_case16_qat_fp16', help='Directory where student models are saved.')
    parser.add_argument('--excel_dir', type=str, default='/home/jijang/ssd_data/projects/Bacteria/excel/case_test/2025_case16_qat_fp16',  help='Directory where excels are saved.')
    ...
    parser.add_argument('--quantization', type=str, choices=['int8', 'fp16'], default='fp16',
                        help='Quantization type of the TFLite model (int8 or fp16)')

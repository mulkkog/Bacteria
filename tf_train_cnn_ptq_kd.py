...
def main(args):
    ...
    for fold_index, (train_index, val_index) in enumerate(kf.split(date_folders)):
        ...
        # Load teacher model (assumed to be a QAT model)
        ...
        # Create student model in full precision (PTQ: no QAT applied)
        ...
        if args.save_best:
            checkpoint_path = os.path.join(args.student_model_dir, f"Fold_{fold_index + 1}_model.h5")
        ...
        # Save student model (if save_best not used, save last model)
        if not args.save_best:
            model_save_path = os.path.join(args.student_model_dir, f"Fold_{fold_index + 1}_model.h5")
        ...
        saved_model_path = os.path.join(args.student_model_dir, f"Fold_{fold_index + 1}_saved_model")
        ...
        # Convert the full precision student model to TFLite with quantization
        ...
        if args.quantization_mode == "int8":
            ...
            quant_mode_suffix = "int8"
        elif args.quantization_mode == "fp16":
            converter.target_spec.supported_types = [tf.float16]
            quant_mode_suffix = "fp16"
        else:
            quant_mode_suffix = "none"
        tflite_model = converter.convert()

        # Save the TFLite model
        tflite_save_path = os.path.join(args.student_model_dir, f"Fold_{fold_index + 1}_ptq_model_{quant_mode_suffix}.tflite")
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Script to train and evaluate a PTQ CNN model with knowledge distillation on image data.")
    ...
    parser.add_argument('--student_model_dir', type=str, default='/home/jijang/ssd_data/projects/Bacteria/models/case_test/2025_case16_ptq_fp16', help='Directory where student models are saved.')
    ...
    parser.add_argument('--quantization_mode', type=str, default='fp16', choices=['none', 'int8', 'fp16'],
                        help='Quantization mode for TFLite conversion: none, int8, or fp16.')
